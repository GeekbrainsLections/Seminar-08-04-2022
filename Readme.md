...
Для того чтобы скопировать удалённый репозиторий на свой ПК необходимо в терминале вызвать следующую команду: *git clone <адрес удаленного репозитория>*. 

Для того чтобы скачать всё из удалённого репозитория необходимо воспользоваться командой *git pull*. Важно отметить, что данная команда скачивает копию текущей ветки с указанного удалённого репозитория и объединяет её с локальной копией.

Для того чтобы отправить свою версию  репозитория на внешний репозиторий необходимо воспользоваться следующей командой: *git push*. Данная команда требует выполнить авторизацию на внешнем репозитории.
